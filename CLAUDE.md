...
| 🤖 Claude活用プロンプト集 | `docs/Claude活用プロンプト集.md`（A〜F カテゴリ別 30本） |
| 💰 価格・プラン制限 | `docs/marketing/Trading_Complete_中核オファー要件定義書_v1_0.md` |
| 💳 決済実装 | `docs/features/決済システム要件定義書_v3_10.md` ⭐ |
| 🤖 Premium AI機能 | `docs/features/TC_Premium_AI機能_要件定義書_v1_3.md`（2026年9月リリース・業務OS戦略） |
...
タスクが下記カテゴリに該当する場合、**`docs/Claude活用プロンプト集.md` を Read tool で読み込んで** 該当プロンプトの構成・品質基準・禁止事項を厳守する（`@` 構文での自動インポートはトークンコストが高いため使わず、必要時のみ Read で読み込むオンデマンド方式）。

| カテゴリ | プロンプト一覧 |
|---|---|
| **A** TC開発系 | A-01 要件定義書 / A-02 デバッグ診断 / A-03 リファクタ計画 / A-04 セキュリティレビュー / A-05 機能仕様書 |
| **B** T-CAX改善系 | B-01 プロンプト統廃合 / B-02 投稿品質診断 / B-03 検証ロジック設計 / B-04 ホットトピック選定 / B-05 事件簿エントリ |
| **C** X投稿系 | C-01 朝の相場ノート / C-02 失敗談投稿 / C-03 Build in Public / C-04 教育型スレッド / C-05 リプライ返信 |
| **D** note記事系 | D-01 体験談 / D-02 ノウハウ / D-03 開発ストーリー / D-04 失敗談 / D-05 月次収益レポート |
| **E** LINE配信系 | E-01 ステップ配信 / E-02 リード獲得 / E-03 リリース告知 / E-04 限定オファー / E-05 関係構築型 |
| **F** マーケ戦略系 | F-01 競合分解 / F-02 ターゲット深掘り / F-03 ファネル設計 / F-04 ホームページ改善 / F-05 X広告 |

**使用時のルール**:
- 出力冒頭に適用したプロンプト番号を明記する（例: `[A-02 適用]`）
- プロンプト集§5「共通プロンプトブロックA〜D」も適宜併用
- 「使い始めの5本」（C-01 / D-04 / A-02 / E-01 / F-02）は最頻出として優先記憶
...
| 段階分割 | 🔴 CRITICAL | 複数タスクを一度に頼まれたら必ず段階分割して確認を取る（事件簿の事件8等の再発防止） |
| 出力スタイル | ⚠️ HIGH | アスタリスク禁止・「〜と思われます」等のアナリスト調禁止（コンパナ口調維持） |
| ファイル編集 | ⚠️ HIGH | 既存ファイルは Edit tool で部分編集（Write での全置換・再生成は新規作成時のみ） |
...
> **思想**: TC は「売って終わり」のSaaSではなく、ユーザーの記録を**宝の地図**に変える業務OS。
> Premium は「コストではなく自己投資」と認識される設計（投資効果ダッシュボード）で、
> 「敵に回すと困るサービス」を構造的に作る（v1.3 §1.1）。

- 2026年夏: 正式リリース（Pro + 早期割引100名）
- **2026年9月: Premium 正式リリース**（v1.3 戦略・3層構造）
  - **主役①** 確定申告AI（経費自動分類・節税額可視化・申告サポート）
  - **主役②** プロアクティブ週次レビュー（毎週日曜21:00自動配信）
  - **脇役** 投資効果ダッシュボード（「コスト→自己投資」の可視化）
- 2026年12月: 個人プレイブック自動生成・規律遵守率ダッシュボード
- 2027年3月: 自然言語Q&A（Phase 2 サブ機能化）
- 2027年6月: スマートアラート（リベンジトレード検出等）
