...
## 🚨 現在の最重要ボトルネック（2026-04-30 時点）

**正式リリースまでの残作業は1つだけ**:

### Square Phase 10 本番切替（約3時間・4-14〜4-19）
PayPal・Square ともに Sandbox テスト全合格済み。**残るは本番APIキーへの切替と本番E2Eテストのみ**。
...
詳細: `docs/features/決済システム要件定義書_v3_10.md` §6 Step 4

### ✅ リリース前ブロッカー解消済み（2026-04-28〜04-30）

| 項目 | 完了日 |
|---|---|
| **Square Phase 10A〜B**（Sandbox統合・E2Eテスト全合格・改善#2 UPSERT化） | 2026-04-28 |
| **Phase 11 サーバー側プラン制限**（RLS+INSERTトリガー+多層防御） | 2026-04-28 |
| **計算ロジック検証・修正フェーズ**（全17 FIX完了 / 検証9/9合格） | 2026-04-30 |

→ 本番切替を終えれば PayPal+Square 二本立てで正式リリース可能。
...
**教訓**: 1つの決済会社に全部賭けない。多層構造で構築する（v3.10 §5）。
...
### 価格・決済の話題が出たら
**必ず `docs/features/決済システム要件定義書_v3_10.md` を最新の正として参照する**。
他のドキュメント（特に `marketing/` 以下）の旧価格（¥2,480/¥3,980等）は無効。

### リリース指揮が必要なら
`/release-go` スキルを使う（`.claude/skills/release-go/SKILL.md`）。
Phase 10 本番切替（4-14〜4-19）の残作業を「今この瞬間にやるべき1〜2件」に変換して提示する。
...
*このファイルは 2026-04-30 にCEO秘書室として整備*
