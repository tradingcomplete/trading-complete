...
| `features/決済システム要件定義書 v3.9.md` | 決済実装上の価格・プランID・Edge Function仕様 |
...
| 変更 | 理由 |
|---|---|
| Stripe決済要件定義書を「価格マスター」から外す | Stripe閉鎖により無効化、`決済システム要件定義書 v3.9` が新マスター |
| 中核オファー要件定義書 価格を ¥1,980/¥2,980 に更新 | v3.9 多層決済戦略に整合 |
...
1. **マスターは2つだけ**: 中核オファー要件定義書 + 決済システム要件定義書 v3.9
