...
**日付**: 2026-02-02 | **工数**: 4h | **優先度**: MEDIUM

**概要**: 年度別利益率と全期間利益率の表示機能を追加

**実装ファイル（MODULES.md準拠）**:

| ファイル | 内容 |
|----------|------|
| SettingsModule.js | 年初口座残高API追加 |
| SyncModule.js | yearStartBalances同期追加 |
| StatisticsModule.js | 分析タブ利益率初期化（EventBus連携） |
| capital-ui.js | 全期間利益率UI追加 |
| year-start-balance-ui.js | 年初口座残高設定UI（新規作成） |

**データ構造**:
- LocalStorage: `yearStartBalances` = `{"2026": 1000000}`
- Supabase: `user_settings.year_start_balances` (JSONB)

**教訓**:
1. MODULES.md準拠を最優先（index.htmlに直接書かない）
2. 設定タブUIは `js/part5/` に配置
3. 初期化タイミングはDOMContentLoaded + EventBusで制御

**関連**: 利益率機能_要件定義書_最終版_v2_0.md
...
設定タブ関連のUIは `js/part5/` に配置：

| ファイル | 内容 |
|----------|------|
| broker-ui.js | ブローカー管理 |
| favorite-pair-ui.js | お気に入り通貨ペア |
| capital-ui.js | 入出金管理 |
| year-start-balance-ui.js | 年初口座残高（2026-02-02追加） |
