...
// ノート画像（URL形式）
note.images = [
  { url: 'https://xxx.supabase.co/storage/v1/object/...', timestamp: '...' }
]
```
...
| 16 | chart_images | jsonb | チャート画像（URL配列） |
...
| chartImages | chart_images | jsonb | 配列形式（URL） |
...
```javascript
content: '{"memo":"...","marketView":"...","images":[{"url":"..."}]}'
```
...
*このドキュメントをPhase 5以降の開発で参照してください。*
