...
| ~~オフライン対応~~ | ~~オフライン時はローカルに保存、復帰時に同期~~ → ✅ 完了（2026-03-09 / SyncModule v1.8.0） |
...
| # | テスト項目 | 状態 | 結果 |
|---|-----------|------|------|
| 1 | オフライン時の動作 | ✅ | 差分マージ機能（SyncModule v1.8.0）により解決。オンライン復帰を自動検知し、updated_atのLast Write Wins方式でトレード・ノートをマージ。経費・入出金はIDユニオン方式。全3シナリオのテスト合格（2026-03-09） |
| 2 | 同時編集時の動作 | ✅ | Last Write Wins（後勝ち）で正常動作 |
| 3 | 大量データ時のパフォーマンス | ✅ | localStorage 2.6%使用、1年後推定637KBで問題なし |

**オフライン対応の詳細（✅ 解決済み 2026-03-09）**:
- オフライン中の操作はlocalStorageに保存される
- オンライン復帰を `window.addEventListener('online', ...)` で自動検知
- `mergeAllWithCloud()` が自動実行され、差分マージを行う
- トレード・ノート: `updated_at` タイムスタンプによるLast Write Wins方式
- 経費・入出金: IDユニオン方式（updated_atなしのため）
- **実装**: SyncModule v1.8.0（新規メソッド5個追加、既存コード変更なし）
...
| オフライン時の操作 | 低 | **差分マージ機能で対応済み** ✅ 対策済み（SyncModule v1.8.0 / 2026-03-09） |
...
| SyncModule.js | v1.8.0 | クラウド同期（5テーブル + Storage + オフライン差分マージ） |
...
|---------|------------------|---------|
| trades | `trades` | リアルタイム + オフラインマージ |
| notes | `notes` | リアルタイム + オフラインマージ |
| expenses | `tc_expenses` | リアルタイム + IDユニオン |
| capital_records | `depositWithdrawals` | リアルタイム + IDユニオン |
...
### オフライン対応（2026-03-09追加）

| 項目 | 実装 |
|------|------|
| 復帰検知 | `window.addEventListener('online', ...)` |
| トレード・ノート | `updated_at` Last Write Wins方式 |
| 経費・入出金 | IDユニオン方式 |
| 自動マージ | `mergeAllWithCloud()` |
...
| **v3.0** | **2026-03-09** | **🎉 完成版 - 全Phase完了、オフライン差分マージ機能追加（SyncModule v1.8.0）** |
