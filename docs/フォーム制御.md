...
| 要素ID | 用途 | 制御ファイル |
|--------|------|-------------|
| `imageCaptionEditModal` | モーダル本体 | script.js |
| `captionEditPreviewImg` | プレビュー画像 | script.js |
| `captionEditTitle` | 題名入力（30文字） | script.js |
| `captionEditDesc` | 説明入力（100文字） | script.js |
| `captionEditTitleCount` | 題名文字数 | script.js |
| `captionEditDescCount` | 説明文字数 | script.js |
| `captionEditSaveBtn` | 保存ボタン | script.js |
| `changeImageInEditBtn` | 画像変更ボタン | script.js |
| `modalCaptionEditBtn` | 拡大モーダル内✏️ボタン | script.js |

**データフロー（新規追加時）**:
```
Step1（画像選択）
  ↓
imageCaptionEditModal（題名・説明入力）
  ↓
saveImageCaptionEdit() → handleProcessedImage()
  ↓
トレード/ノートに保存
```
...
拡大モーダル ✏️クリック
  ↓
openModalImageEdit() → openImageCaptionEdit()
  ↓
imageCaptionEditModal表示（既存データ読込）
  ↓
saveImageCaptionEdit() → 更新
```
...
window.pendingImageType = imageType（tradeChart1/2/3, noteImage）
  ↓
画像選択 → imageCaptionEditModal（題名・説明入力）
  ↓
saveImageCaptionEdit() → handleProcessedImage(imageData)
  ↓
pendingImageTypeで分岐:
...
```javascript
{
  ...
  path: 'userId/trades/.../chart1.jpg',  // 復元用
  title: 'エントリー時の日足',
...
### 画像モーダル統合・デザイン統一（2026-01-23）

**実施内容**: 画像追加Step2を削除し、imageCaptionEditModalに統合

**統合前**:
- Step1: 画像選択
- Step2: 題名・説明入力（imageAddStep2）
- 別途: imageCaptionEditModal（編集用）

**統合後**:
- Step1: 画像選択
- imageCaptionEditModal: 追加/編集両対応

**修正されたバグ**:

1. **pendingImageTypeリセット問題**
   - 原因: `closeImageAddModal`が早期にリセット
   - 修正: リセットタイミングを適切化

2. **二重ネスト問題**
   - 原因: `createImageData`にオブジェクトを渡す
   - 修正: `getImageSrc`で実際のソースを取得

3. **相場ノート画像編集「画像が見つかりません」**
   - 原因: NoteManagerModuleが旧形式を返す
   - 修正: `tempNoteEditImages`から優先取得
