...
> **価格・プラン制限の正**: `docs/features/決済システム要件定義書 v3.9.md` を最新マスターとする
...
**詳細**: `docs/features/決済システム要件定義書 v3.9.md` ← 最新マスター
...
| **10** | **Square実装（Subscriptions API → Edge Functions → PaymentModule統合）** | **🔄 進行中**: Edge Functions 3つ デプロイ済み（4-1〜4-8完了）／フロントエンド統合は未着手（4-9〜4-19残・約25h） | - |
...
### Phase 10 残作業（Square フロントエンド統合 / 約25h）
...
v3.9 §11 を参照。**フロント側のチェックは残し、サーバー側を「追加」する多層防御**。

| # | タスク | 所要時間 | 状態 |
|---|--------|---------|------|
| 5-1 | フロント側制限ロジックの全箇所棚卸し（PaymentModule.canAddTrade等） | 1時間 | ⬜ |
| 5-2 | サーバー側で守るべき制限の確定（件数・クラウド同期・Storage・将来AI） | 1時間 | ⬜ |
| 5-3 | subscriptions テーブルとの連携設計（プラン取得関数） | 1時間 | ⬜ |
| 5-4 | trades テーブルに INSERT トリガー実装（20件制限） | 2時間 | ⬜ |
| 5-5 | Storage Policy にプラン別ルール追加（画像アップロード制限） | 1時間 | ⬜ |
| 5-6 | RLS Policy 整備（プラン別 SELECT/INSERT/UPDATE 制限） | 2時間 | ⬜ |
| 5-7 | フロントエンドのエラーハンドリング更新 | 1時間 | ⬜ |
| 5-8 | テスト1: 正常系（Free=20件まで保存、Pro=無制限） | 1時間 | ⬜ |
| 5-9 | テスト2: 攻撃系（F12突破でも21件目はDB拒否） | 1時間 | ⬜ |
| 5-10 | テスト3: プラン変更時の即時反映 | 30分 | ⬜ |

**根拠**: 現状の PaymentModule.canAddTrade はブラウザJSなのでF12で書き換え可能。
技術力のあるユーザーがFreeのまま無制限利用できる脆弱性を、Supabase RLS+トリガーで塞ぐ。
...
**料金プラン**: Free / Pro / Premium の3プラン構成（v3.9確定）
※価格・制限・早期割引の正は「決済システム要件定義書 v3.9」と「中核オファー要件定義書」を参照
※決済プロバイダの実装は v3.9 が一次資料
...
| 追加決済手段の検討 | 必要に応じて | FastSpring / PAY.JP再申請 等を v3.9 多層戦略に従って検討 |
...
| タスク | 優先度 | 工数 | 備考 |
|--------|--------|------|------|
| **Square Phase 10残（4-9〜4-19）** | **CRITICAL** | **約25h** | **リリース前必須・決済システム要件定義書 v3.9 §6 Step 4** |
| **Phase 11 サーバー側プラン制限** | **CRITICAL** | **約12h** | **リリース前必須・F12突破対策・v3.9 §11** |
