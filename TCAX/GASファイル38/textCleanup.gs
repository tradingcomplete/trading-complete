/**
 * CompanaFXAutoPost - textCleanup.gs
 * テキスト清掃系の後処理関数群(applyPostProcessingChain_ が順次呼ぶ)
 *
 * 提供する関数(applyPostProcessingChain_ の実行順):
 *   - removeForeignText_:             外国語混入除去
 *   - stripAIPreamble_:               AI前置き除去(「承知しました」「以下が〜」等)
 *   - enforceLineBreaks_:             改行整形
 *   - removeDisallowedEmoji_:         禁止絵文字除去(☕📕📝📋💡⚠️✅ 以外)
 *   - fixOrphanedVariationSelector_:  孤立 U+FE0F(異体字セレクタ)修復
 *   - removeMarkdown_:                Markdown記法除去
 *   - replaceProhibitedPhrases_:      禁止表現置換
 *   - fixIncompleteVerbEnding_:       文末未完結修正(「〜かなと。」「〜とこ。」等)
 *   - fixMondayYesterday_:            月曜日の「昨日」「昨夜」修正
 *   - removeDuplicateBlocks_:         重複ブロック除去
 *   - removeOrphanedLines_:           孤立短文除去
 *   - fixBrokenSentenceEndings_:      壊れた句点パターン修復
 *   - truncateAfterHashtag_:          ハッシュタグ後ろ切断
 *
 * 設計の鉄則(postProcessor.gs 元ヘッダから継承):
