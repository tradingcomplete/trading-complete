...
function selectHotTopic_(params) {
  ...
  try {
    if (priorPosts && priorPosts.length > 0) {
      ...
      prompt += '   - 当日最大材料(日銀・FOMC等)を扱う場合でも 1 日に最大 2 回まで\n';
      prompt += '   - 同じ材料を扱う場合は、必ず別の角度・別の通貨ペア・別の時間軸から切り込め\n';
      prompt += '   - 切り口の例:\n';
      prompt += '     朝に「日銀会合・植田発言」をドル円で書いたなら、\n';
      prompt += '     昼は「豪ドル独歩高」をリスクオン文脈で別ペア視点で、\n';
      prompt += '     夕は「欧州勢の反応」を GBP/EUR で語る、など\n';
      prompt += '   - 完全に同じ主題が当日 3 投稿目以降に出た場合は、必ず別主題を選べ\n';
      prompt += '\n';
    }
  } catch (priorErr) {
