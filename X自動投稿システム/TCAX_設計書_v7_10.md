...
**v7.10変更点**: 壊れた句点パターン修復+孤立variation selector修復+Claude API 529対策（対策A+B+C+D）。fixBrokenSentenceEndings_新設（「。ですね。」等を汎用修復）。replaceProhibitedPhrases_に「ここからが本番。」壊れパターン除去追加。fixOrphanedVariationSelector_新設（⚠️ベース文字欠落でU+FE0Fだけ残る問題を修復）。miReplacements「ここからが本番」→「ここが勝負どころ」に変更（根本原因修正）。callClaude_改修（指数バックオフ5→10→20秒、リトライ2→3回、エラー詳細ログ）。postProcessor.gs 1,898→1,977行（22→24関数）、qualityReview.gs 411→427行
**v7.9変更点**: ニュース主軸ルール追加。buildMarketTypePolicy_に「投稿の主軸はニュース。レートは裏付け」ルール+OK/NG例追加。buildFormatRules_の題材選定・投稿構造をニュース優先に更新。promptBuilder.gs 1,749→1,759行
**v7.8変更点**: charMax緩和（300→380/350→420/450→550）、答え合わせ重複防止（getHypothesisContext_にキャッシュ照合追加）、品質レビュー全面改修（Claude指摘のみ→Gemini修正→trimToCharMax_文字数保証）、孤立短文除去（removeOrphanedLines_新設）。config.gs charMax更新、promptBuilder.gs 1,722→1,749行、qualityReview.gs 332→411行、postProcessor.gs 1,832→1,898行
**v7.7変更点**: 条件分岐型仮説ルール追加（6箇所）。一方向の賭け仮説を禁止し「もしAならB、もしCならD」の読み解き型を推奨。promptBuilder.gs 1,709→1,722行
**v7.6変更点**: buildPrompt_分割リファクタリング（636行→391行）。5つのヘルパー関数新設（buildMarketTypePolicy_/buildKnowledgePolicy_/buildIndicatorPolicy_/buildWeekendPolicy_/buildRulePolicy_）。promptBuilder.gs 1,650→1,709行・17関数。今後の方針変更はヘルパー関数内を修正するだけ
**v7.5変更点**: 未発表指標の仮説答え合わせ防止（二重防御）。対策A: GOLDEN/NY構造指示ルール追加。対策B: getUnreleasedIndicatorNames_新設。重複コメント6箇所除去。postProcessor.gs 1,837→1,832行、promptBuilder.gs 1,581→1,650行
...
promptBuilder.gs          1,759   18     プロンプト構築: ★v8.13: ニュース主軸+v8.12: 答え合わせ重複防止+条件分岐型仮説
postProcessor.gs          1,977   24     後処理チェーン: ★v8.14: fixBrokenSentenceEndings_+fixOrphanedVariationSelector_新設
factCheck.gs              660     5      ファクトチェック: ★v8.7: 要人リスト確定データ注入、ルール文言変更
qualityReview.gs          427     7      ★v8.14: callClaude_指数バックオフ+リトライ3回+エラー詳細ログ
