...
・Gemini API                 → テキスト生成（gemini-2.5-flash）+ 画像生成（gemini-3-pro-image-preview）★v8.0モデル更新
...
│
├── geminiApi.gs          ← 核: メイン処理+Gemini API通信 ✅実装済み（638行）★v8.6: 検証不能❌即削除+品質レビュー統合
...
│
├── promptBuilder.gs      ← プロンプト構築+データ注入    ✅実装済み（1,536行）★v8.6: タイプ別条件分岐+参照ソース修正+ソース元言及
...
│
├── postProcessor.gs      ← 後処理チェーン              ✅実装済み（1,745行）★v8.6: fixHallucinatedRates_修正+normalizeRateDecimals_+呼びかけ除去
...
│
├── testFunctions.gs      ← テスト関数                  ✅実装済み（819行）★v8.5分割
├── qualityReview.gs      ← 品質レビュー（Claude API）   ✅実装済み（332行）★v8.6新規追加
│   ├── testAll1〜6()        分割バッチテスト（全16タイプ）
│   ├── testBatch_()         バッチテスト実行エンジン
│   └── testGemini() / testFetchRates() / testSheetLoading() 等
│
...
│   ├── GEMINI_MODEL         テキスト生成モデル（gemini-2.5-flash）★v8.0更新
│   ├── SCHEDULE             曜日別1分単位時刻
...
【スキップ対象】
  ...
  ・旧testGenerateAll: 廃止（★v8.0でtestAll1〜6に再分割しファクトチェック込みに変更）
  ・testAll1〜6: ファクトチェック込み（3タイプ×6グループ、最長約5分で6分制限に余裕）★v8.0
...
testAll1()                  分割テスト グループ1（MORNING/TOKYO/LUNCH）ファクトチェック込み ✅安全 ★v8.0再分割
testAll2()                  分割テスト グループ2（LONDON/GOLDEN/NY）ファクトチェック込み ✅安全 ★v8.0再分割
testAll3()                  分割テスト グループ3（INDICATOR/KNOWLEDGE/WEEKLY_REVIEW）ファクトチェック込み ✅安全 ★v8.0再分割
testAll4()                  分割テスト グループ4（RULE_1/RULE_2/RULE_3）ファクトチェック込み ✅安全 ★v8.0再分割
testAll5()                  分割テスト グループ5（RULE_4/WEEKLY_LEARNING）ファクトチェック込み ✅安全 ★v8.0新規
testAll6()                  分割テスト グループ6（NEXT_WEEK/WEEKLY_HYPOTHESIS）ファクトチェック込み ✅安全 ★v8.0新規
...
【Gemini API】★v8.0モデル更新
  テキスト生成: gemini-2.5-flash（Stable版）
