...
function buildPrompt_(postType, typeConfig, context, rates) {
  ...
  if (marketTypes.indexOf(postType) !== -1) {
    prompt += '\n【市場系投稿の方針（最重要）】\n';
    prompt += '■ 指標に言及する場合は時刻を含めて正確に。上記「今日の経済指標」の時刻を使え。\n';
    ...
    prompt += '■ コンパナの投稿はニュース要約ではない。「仮説の提示→答え合わせ→次の仮説」のサイクルで回せ。\n';
    prompt += '■ 前回の仮説の振り返りデータが注入されている場合、必ず1ブロック使って答え合わせを入れろ。\n';
    prompt += '■ 仮説は「非自明な読み」を出せ。以下を厳守:\n';
    prompt += '  ・「指標上振れなら通貨高」のような当たり前の仮説は禁止。誰でも言える。\n';
    prompt += '  ・政治（トランプの関税、各国の選挙）、地政学（中東、ホルムズ海峡）、構造変化（高齢化、財政赤字）に根ざした仮説が理想。\n';
    prompt += '  ・「表面上は○○だが、実は○○ではないか」「市場はまだ○○を織り込んでいない」の視点。\n';
    prompt += '  ・為替の裏にある人間社会・生活への影響にも触れろ（円安と海外旅行、原油高とガソリン代等）。\n';
    prompt += '  ・OK: 「停戦報道で楽観ムードだが、イランの本音は時間稼ぎ。この楽観は脆い」\n';
    prompt += '  ・OK: 「市場は利下げ織り込みを進めているが、トランプの関税第2弾が来たらインフレ再燃でシナリオ崩壊」\n';
    prompt += '  ・NG: 「失業保険が予想以下ならドル買い」（当たり前すぎる）\n';
    prompt += '  ・NG: 「159.50超えならロング。損切は159.00」（数字の羅列。読者の目が滑る）\n';
    prompt += '■ 負けた（外れた）時は正直に書け。「読み違えた」「反省ポイント」も投稿の価値。\n';
    prompt += '■ ★「トレードした」「エントリーした」「利確した」は禁止（実際にトレードしていない）。\n';
    prompt += '  ・「自分はこう読んでいる」「こう見ている」「こういう展開を想定している」で表現。\n';
    prompt += '\n【レートデータの使い方（最重要）】\n';
    prompt += '■ 注入されたレート・指標データは「方向感の把握」に使え。数字を投稿に並べる目的ではない。\n';
    prompt += '■ OK: 「ドル円は上昇基調が続いていて」「ユーロは先週から売られっぱなし」\n';
    prompt += '■ NG: 「ドル円は159.27円」「EUR/USDは1.1568ドル」（数字の羅列）\n';
    prompt += '■ レート数字を使うのは、大台（160円）や大きな変動幅の説明時だけに限定せよ。\n';
    prompt += '\n【経済イベント・指標の日付確認ルール（最重要）】\n';
    prompt += '・「〜が控えています」「〜の発表があります」と書く場合、必ず上記の経済カレンダーで今日の日付を確認せよ。\n';
  ...
  if (postType === 'MORNING') {
    ...
    prompt += '■ 構成の基本フレーム:\n';
    prompt += '  ①【仮説答え合わせ】前回の仮説データが注入されていれば冒頭で。正直に。\n';
    prompt += '  ②【昨夜の世界で何が起きたか】NYの数字ではなく、政治・地政学・中銀の姿勢変化を語れ。\n';
    prompt += '  ③【今日の仮説】非自明な読み。「表面上は○○だが実は○○」の視点。当たり前の指標予想は禁止。\n';
    prompt += '■ 【通貨ペア混同禁止】同じ段落で異なる通貨ペアのレートを混ぜるな。\n';
  }
  
  // ②-c1.6 TOKYO共通: 東京市場序盤の認識（全曜日共通） ★v6.6追加
  if (postType === 'TOKYO') {
    prompt += '\n【TOKYO投稿の時間帯と役割（重要 - 全曜日共通）】\n';
    prompt += '■ 朝9:11〜9:43頃配信。東京オープン後10〜40分。「予想」ではなく「観察」。\n';
    prompt += '■ MORNINGの仮説の「途中経過」を1文入れろ。\n';
    prompt += '■ 東京勢の空気感を描写（実需の動き、輸出勢の売り等）。数字ではなく行動を語れ。\n';
    prompt += '■ 8:50の日本指標が発表済みなら結果に触れよ。\n';
    prompt += '■ 【通貨ペア混同禁止】同じ→の前後で異なる通貨ペアのレートを引用するな。\n';
  }
  
  // ②-c1.7 GOLDEN: 1日のスコアカード ★v8.8.1追加
  if (postType === 'GOLDEN') {
    prompt += '\n【GOLDEN投稿の時間帯と役割（重要）】\n';
    prompt += '■ 夜20-21時台配信。居酒屋トークの温度感。\n';
    prompt += '■ ①仮説スコアカード: 朝の仮説がどうなったか。当たり外れを正直に。これが冒頭。\n';
    prompt += '■ ②今日一番考えさせられたこと: 数字ではなく「世界の動き」から何を感じたか。\n';
    prompt += '■ ③明日への視点: 今日の結果を踏まえて世界がどう動きそうか。\n';
  }
  
  // ②-c1.8 NY: 今夜の焦点と仮説 ★v8.8.1追加
  if (postType === 'NY') {
    prompt += '\n【NY投稿の時間帯と役割（重要）】\n';
    prompt += '■ 夜22時台配信。NY市場オープン前の緊張感。\n';
    prompt += '■ ①今日の仮説進捗: 朝からの読みがどうなっているか。\n';
    prompt += '■ ②今夜の焦点: 指標の「本質」を語れ。数字の予想値ではなく「この結果が出たら世界がどう変わるか」。\n';
    prompt += '■ ③今夜の仮説: 翌朝の答え合わせ前提。非自明な読みを残せ。\n';
    prompt += '  → OK: 「本当に大事なのは継続受給者数の方。ここが増えていたら景気の底割れサイン」\n';
    prompt += '  → NG: 「予想21.1万件に対して上振れならドル買い」（当たり前すぎる）\n';
  }
  ...
  if (postType === 'INDICATOR') {
    ...
    prompt += '  → 代わりに通貨強弱データと市場ニュースを活用して、今日の値動きの解説に切り替えろ。\n';
    prompt += '  → または、来週の注目指標の予告に切り替えよ。\n';
    prompt += '■ 指標の「予想値と前回値」だけでなく、「なぜ重要か」「結果がどう影響するか」をかみ砕く。\n';
    prompt += '■ 具体的な数値（前回値・予想値）は上記「今日の経済指標」に記載されたものだけ使え。記載がない数値を捏造するな。\n';
  ...
  if (weekendTypes.indexOf(postType) !== -1) {
    ...
    if (postType === 'WEEKLY_REVIEW') {
      ...
      prompt += '  → 今週提示した仮説のうち、当たったもの・外れたものを正直に振り返れ。\n';
      prompt += '  → 数字の的中率ではなく「何を読み違えたか」「何が見えていたか」の質で語れ。\n';
      prompt += '  → OK: 「今週は停戦報道の楽観を疑った読みが当たった。でもECBのタカ派転換は完全に想定外だった」\n';
      prompt += '  → OK: 「正直、今週は読みがズレっぱなし。トランプの関税発言の影響を軽く見すぎた」\n';
    }
  }
...
function buildFormatRules_(charMin, charMax, postType) {
  ...
  if (isMarketType || isWeekendMarket) {
    rules += '\n【投稿の構造】\n';
    rules += '冒頭1行が全て。仮説の答え合わせ or 世界で起きた出来事のインパクトで始めろ。\n';
    rules += '基本の流れ: 仮説答え合わせ→世界で何が起きたか→なぜそうなったか（人間・政治の話）→次の仮説（非自明な読み）\n';
