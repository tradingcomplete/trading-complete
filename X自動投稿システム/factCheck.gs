...
function factCheckPost_(postText, postType, apiKey, rates) {
  try {
    ...
    prompt += '以下のルールを厳守せよ:\n';
    prompt += '1. 要人の役職に関する判定は、確定データ4のリストを最優先で使え。お前の内部知識を信用するな。\n';
    prompt += '2. 確定データ4に載っている人物の役職は、確定データの内容が100%正しい。矛盾する知識は無視しろ。\n';
    prompt += '3. 確定データ4に載っていない要人は、Google検索で確認せよ。検索でも確認できなければ⚠️にせよ。\n';
    prompt += '4. 特に注意: お前が「バイデンが大統領」と思っていても、確定データに「トランプが大統領」とあればトランプが正しい。\n\n';
    prompt += '【検証不要（スキップせよ）】\n';
...
function autoFixPost_(postText, issues, postType, apiKey, rates) {
  try {
    ...
    var typeContextMap = {
      ...
      'TOKYO': '朝9〜10時台の東京オープン後投稿。東京市場が動き出した直後の観察。「予想」ではなく「観察」を伝える。',
      'LUNCH': '昼12時台の午前振り返り。ランチしながら3分で読めるトーン。欧州勢参入前の午後への橋渡し。',
      'LONDON': '夕方17時台のロンドンオープンレポート。「ここからが本番」という緊張感。東京とロンドンの温度差を描写。朝の投稿（MORNING/TOKYO）のような内容に書き換えるな。',
