...
## 1. 現状の学習ループ（v12.2 3段階生成+SH/SLダウ理論）
...
★チェック1: ファクトチェック（Gemini 2.5 Pro + Grounding ON）
    → Layer 1: 確定データ照合（レート + BTC/ゴールド + カレンダー + 要人 + アノマリー）★v12.1.1: WTI/天然ガス停止
...
(2) factCheckPost_（Gemini + Grounding ON）← チェック1
    → Layer 1: システム確定データと照合
      ・為替レート（7ペア）: 3%超の乖離で❌
      ・★v12.0: 商品価格（BTC/ゴールド）: 5%超の乖離で❌ ★v12.1.1: WTI/天然ガス停止
      ・政策金利・カレンダー・要人・アノマリー
    → Layer 2: Grounding検索で事実確認
...
【検証する事実】
  ...
  ・レート水準: 具体的な数値の正確性
  ・★v12.0: 商品価格: BTC・ゴールドの具体値 ★v12.1.1: WTI/天然ガス停止（Alpha Vantageデータ古いため）
