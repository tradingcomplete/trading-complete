...
ファイル                  行数    役割
────────────────────────────────────────────────────
geminiApi.gs              1,893   核: generatePost + Claude市場分析 + finalFactVerify_ + ★v12.7: collectAnchorData_ + Step 4配線
promptBuilder.gs          2,116   プロンプト構築 ★v12.9: buildEveningMaterialHint_新設(GOLDEN用) + toFixed→formatRate_置換
postProcessor.gs          2,130   後処理チェーン14段階 ★v12.6: fixHallucinatedRates_クロス汚染修正 + convertExactRatesToRange_新設
factCheck.gs              533     ファクトチェック ★タスク18で削除予定
qualityReview.gs          625     品質レビュー+修正 ★v12.9: Q6確定データ強化(collectAnchorData_呼び出し + 判定優先順位明示)
anomalyManager.gs         1,041   アノマリー自動判定 + 祝日通知
config.gs                 773     設定値 + POST_TYPES + formatRate_ + INTERACTIVE_VERIFY_ENABLED ★v12.9: SCHEDULE整合性修正
rateManager.gs            1,635   レート取得・保存・検証
marketAnalysis.gs         894     市場分析（通貨強弱・ニュース・背景フィールド）
...
testFunctions.gs          656     ★v12.9: Lv2拡充(260→656行、+15関数)。運用監視・3シート確認・Q6シナリオ別・Phase 3分割テスト
★testPostProcessor.gs     392     後処理チェーン不変条件テスト（32ケース）
imageGenerator.gs         898     AI画像生成 + 透かし合成
scheduler.gs              722     トリガー管理 ★v12.5: サマータイムオフセット適用
main.gs                   1,440   ★v12.8: handleManual/Validate/AutoMode_統一 + Phase A/B/C 3関数
sheetsManager.gs          2,059   スプレッドシート読み書き ★v12.7: 対話型検証ログ + 未来日付ガードログ + 継続中重大事象
approval.gs               1,119   承認フロー + メール送信
xApi.gs                   654     X API v2投稿（OAuth 1.0a）
applyPairColors.gs        245     通貨ペア色分け
utils.gs                  172     ユーティリティ

★v12.7 新規追加（タスク17-b, 17-c）:
interactiveVerify.gs       819   ★NEW: 対話型検証本体（Step1-3の3関数）
testInteractiveVerify.gs 1,180   ★NEW: 対話型検証テスト基盤（19関数・11テストケース）
────────────────────────────────────────────────────
合計: 25ファイル・約25,000行

★v12.8 GAS反映済み(2026-04-17 16:47時点):
  ・main.gs: 1,440行（タスク5-a/b/c 完了・3ハンドラー統一）
  ・geminiApi.gs: 1,893行（collectAnchorData_ + Step 4配線済み）
  ・interactiveVerify.gs: 819行（Step1-3実装完了・初稼働成功）
  ・sheetsManager.gs: 2,059行（3シート機能完成）
```
