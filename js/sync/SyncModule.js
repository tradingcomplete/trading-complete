...
(function() {
    ...
    class SyncModuleClass {
        ...
        async initialize() {
            try {
                ...
                // オフライン→オンライン復帰時の自動マージ
                window.addEventListener('online', () => {
                    console.log('[SyncModule] オンライン復帰を検知。差分マージを開始します...');
                    this.mergeAllWithCloud().catch(err => {
                        console.error('[SyncModule] オンライン復帰マージエラー:', err);
                    });
                });
        ...
        /**
         * オフライン中のローカル変更をクラウドとマージ（全テーブル一括）
         * オフライン→オンライン復帰時に自動実行される
         * @returns {Promise<Object>} { success, trades, notes, expenses, capital }
         */
        async mergeAllWithCloud() {
            if (!this.#initialized) {
                console.warn('[SyncModule] mergeAllWithCloud: 未初期化のためスキップ');
                return { success: false, error: '未初期化' };
            }
            
            if (this.#syncInProgress) {
                console.warn('[SyncModule] mergeAllWithCloud: 同期中のためスキップ');
                return { success: false, error: '同期中です' };
            }
        ...
        /**
         * トレードのマージ（Last Write Wins by updatedAt）
         * - ローカルにのみある → クラウドへアップロード
         * - クラウドにのみある → ローカルへ追加
         * - 両方にある → updatedAt が新しい方を採用
         * @returns {Promise<Object>} { success, uploaded, downloaded, kept }
         */
        async mergeTradesWithCloud() {
            if (!this.#initialized) {
                return { success: false, error: '未初期化' };
            }
            ...
            try {
                // クラウドから全トレード取得
                const cloudResult = await this.fetchAllTrades();
                ...
                // ローカルから全トレード取得
                const localTrades = StorageValidator.safeLoad('trades', [], StorageValidator.isTradesFormat);
                const cloudTrades = cloudResult.data;
                
                // IDをキーにしたMapを作成
                const localMap = new Map(localTrades.map(t => [t.id, t]));
                const cloudMap = new Map(cloudTrades.map(t => [t.id, t]));
                
                const merged = [];
                const toUpload = [];  // ローカルにしかない（クラウドへアップロード）
                
                // ローカルのトレードを処理
                for (const [id, localTrade] of localMap) {
                    if (!cloudMap.has(id)) {
                        // ローカルにしかない → クラウドへアップロード対象
                        merged.push(localTrade);
                        toUpload.push(localTrade);
                    } else {
                        // 両方にある → updatedAt で比較
                        const cloudTrade = cloudMap.get(id);
                        const localTime = new Date(localTrade.updatedAt || localTrade.createdAt || 0).getTime();
                        const cloudTime = new Date(cloudTrade.updatedAt || cloudTrade.createdAt || 0).getTime();
                        
                        if (localTime > cloudTime) {
                            // ローカルが新しい → ローカルを採用してクラウドへアップロード
                            merged.push(localTrade);
                            toUpload.push(localTrade);
                        } else {
                            // クラウドが新しい（または同時刻）→ クラウドを採用
                ...
                for (const [id, cloudTrade] of cloudMap) {
                    if (!localMap.has(id)) {
                        merged.push(cloudTrade);
                        downloadedCount++;
                    }
                ...
                // ローカルにしかないものをクラウドへアップロード
                for (const trade of toUpload) {
                    try {
                        await this.saveTrade(trade);
                    } catch (e) {
                        console.warn('[SyncModule] トレードアップロードスキップ:', trade.id, e);
                    }
                }
        ...
        /**
         * ノートのマージ（Last Write Wins by updatedAt）
         * - ローカルにのみある日付 → クラウドへアップロード
         * - クラウドにのみある日付 → ローカルへ追加
         * - 両方にある日付 → updatedAt が新しい方を採用
         * @returns {Promise<Object>} { success, uploaded, downloaded }
         */
        async mergeNotesWithCloud() {
            if (!this.#initialized) {
                return { success: false, error: '未初期化' };
            }
            ...
            try {
                // クラウドから全ノート取得
                const cloudResult = await this.fetchAllNotes();
                if (!cloudResult.success) {
                    return { success: false, error: cloudResult.error };
                }
                
                // ローカルから全ノート取得（{YYYY-MM-DD: {memo, marketView, ...}} 形式）
                const localNotes = StorageValidator.safeLoad('notes', {}, StorageValidator.isObject);
                const cloudNotes = cloudResult.data; // 同形式
                
                const merged = { ...cloudNotes }; // まずクラウドをベースにする
                const toUpload = [];
                let downloadedCount = 0;
                
                // ローカルのノートを処理
                for (const [date, localNote] of Object.entries(localNotes)) {
                    if (!cloudNotes[date]) {
                        // ローカルにしかない → マージ結果に追加 & アップロード対象
                        merged[date] = localNote;
                        toUpload.push({ date, note: localNote });
                    } else {
                        // 両方にある → updatedAt で比較
                        const localTime = new Date(localNote.updatedAt || localNote.createdAt || 0).getTime();
                        const cloudTime = new Date(cloudNotes[date].updatedAt || cloudNotes[date].createdAt || 0).getTime();
                        
                        if (localTime > cloudTime) {
                            // ローカルが新しい → ローカルを採用してアップロード
                            merged[date] = localNote;
                            toUpload.push({ date, note: localNote });
                        }
                        // クラウドが新しい場合は merged のクラウド値がそのまま残る
                    }
                }
                
                // クラウドにしかないものをカウント
                for (const date of Object.keys(cloudNotes)) {
                    if (!localNotes[date]) {
                        downloadedCount++;
                    }
                }
                ...
                // ローカルにしかないものをクラウドへアップロード
                for (const { date, note } of toUpload) {
                    try {
                        await this.saveNote(date, note);
                    } catch (e) {
                        console.warn('[SyncModule] ノートアップロードスキップ:', date, e);
                    }
                }
        ...
        /**
         * 経費のマージ（IDユニオン方式 - updated_atなし）
         * - ローカルにのみある → クラウドへアップロード & ローカルに保持
         * - クラウドにのみある → ローカルへ追加
         * - 両方にある → クラウド優先（通常は同じデータのはず）
         * @returns {Promise<Object>} { success, uploaded, downloaded }
         */
        async mergeExpensesWithCloud() {
            if (!this.#initialized) {
                return { success: false, error: '未初期化' };
            }
            ...
            try {
                // クラウドから全経費取得
                const cloudResult = await this.fetchAllExpenses();
                if (!cloudResult.success) {
                    return { success: false, error: cloudResult.error };
                }
                ...
                // ローカルにしかないものをクラウドへアップロード
                const toUpload = localExpenses.filter(e => !cloudIds.has(e.id));
                for (const expense of toUpload) {
                    try {
                        await this.saveExpense(expense);
                    } catch (e) {
                        console.warn('[SyncModule] 経費アップロードスキップ:', expense.id, e);
                    }
                }
                
                // クラウドにしかないもの
                const toDownload = cloudExpenses.filter(e => !localIds.has(e.id));
                
                // マージ: クラウド全件 + ローカルにしかないもの
                const merged = [...cloudExpenses, ...toUpload];
        ...
        /**
         * 入出金記録のマージ（IDユニオン方式 - updated_atなし）
         * - ローカルにのみある → クラウドへアップロード & ローカルに保持
         * - クラウドにのみある → ローカルへ追加
         * - 両方にある → クラウド優先
         * @returns {Promise<Object>} { success, uploaded, downloaded }
         */
        async mergeCapitalRecordsWithCloud() {
            if (!this.#initialized) {
                return { success: false, error: '未初期化' };
            }
            ...
            try {
                // クラウドから全入出金取得
                const cloudResult = await this.fetchAllCapitalRecords();
                if (!cloudResult.success) {
                    return { success: false, error: cloudResult.error };
                }
                
                const localRecords = StorageValidator.safeLoad('depositWithdrawals', [], StorageValidator.isArray);
                const cloudRecords = cloudResult.data;
                
                const cloudIds = new Set(cloudRecords.map(r => r.id));
                const localIds = new Set(localRecords.map(r => r.id));
                
                // ローカルにしかないものをクラウドへアップロード
                const toUpload = localRecords.filter(r => !cloudIds.has(r.id));
                for (const record of toUpload) {
                    try {
                        await this.saveCapitalRecord(record);
                    } catch (e) {
                        console.warn('[SyncModule] 入出金アップロードスキップ:', record.id, e);
                    }
                }
                
                // クラウドにしかないもの
                const toDownload = cloudRecords.filter(r => !localIds.has(r.id));
                
                // マージ: クラウド全件 + ローカルにしかないもの
                const merged = [...cloudRecords, ...toUpload];
    ...
    console.log('[SyncModule] モジュール読み込み完了 v1.8.0');
